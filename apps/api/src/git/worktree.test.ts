...
describe("worktree", () => {
  it("creates, lists, and removes a worktree", async () => {
    ...
    await fs.mkdir(path.join(repoDir, "apps", "web", "node_modules"), {
      recursive: true,
    });
    ...
    expect(
      (await fs.lstat(path.join(wtPath, "node_modules"))).isSymbolicLink(),
    ).toBe(true);
    expect(
      (
        await fs.lstat(path.join(wtPath, "apps", "web", "node_modules"))
      ).isSymbolicLink(),
    ).toBe(true);
  ...
  it("can skip dependency hydration for merge-only worktrees", async () => {
    ...
    const wtPath = path.resolve(
      `${repoDir}/../run-r1-${path.basename(repoDir)}`,
    );
    await addWorktreeForBranch(repoDir, wtPath, "run/r1", {
  ...
  it("fast-forwards the user branch to the completed run branch while ignoring .orc state", async () => {
    ...
    expect(await fs.readFile(path.join(repoDir, "index.html"), "utf8")).toBe(
      "<h1>Cows</h1>\n",
    );
  });
  ...
  it("commits modified tracked files without truncating the first path from porcelain status output", async () => {
    ...
    expect(
      execFileSync("git", ["show", "HEAD:track.md"], {
        cwd: repoDir,
        encoding: "utf8",
      }),
    ).toBe("track draft\n");
