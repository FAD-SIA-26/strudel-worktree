...
interface InitTestRepoOptions {
  initialBranch?: string
}

export function initTestRepo(label = `${Date.now()}`, opts: InitTestRepoOptions = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `orc-test-${label}-`))
  execSync(`git init -b ${opts.initialBranch ?? 'main'}`, { cwd: dir, stdio: 'pipe' })
