...
describe("MastermindStateMachine", () => {
  it("decomposes → spawns leads in dependency order → OrchestrationComplete", async () => {
    ...
    const events: string[] = [];
    eventBus.on("event", (e: OrcEvent) => events.push(e.eventType));
    ...
    const m = new MastermindStateMachine({
      ...
      llmCall: async (p) => {
        ...
        if (p.includes("Generate"))
          return JSON.stringify(["write bass pattern"]);
  ...
  it("passes the starting branch through to workers instead of assuming main", async () => {
    repoDir = initTestRepo("mastermind-branch-test", {
      initialBranch: "trunk",
    });
    const db = createTestDb();
    const gov = new ConcurrencyGovernor(4);
    ...
    const m = new MastermindStateMachine({
      ...
      llmCall: async (p) => {
        ...
        if (p.includes("Generate"))
          return JSON.stringify(["build the landing page"]);
        return JSON.stringify({
          winnerId: "r2-landing-v1",
