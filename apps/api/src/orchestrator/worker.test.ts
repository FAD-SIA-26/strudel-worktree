...
describe("WorkerStateMachine", () => {
  it("commits successful worker changes onto the worker branch", async () => {
    ...
    expect(
      execFileSync("git", ["show", "HEAD:RESULT.txt"], {
        cwd: wtPath,
        encoding: "utf8",
      }),
  ...
  it("records heartbeats in the runs table for active workers", async () => {
    ...
    expect(result.status).toBe("done");
    const runRow = getSQLite(db)
      .prepare("SELECT pid, last_seen_at FROM runs WHERE entity_id=?")
      .get("rhythm-v4") as { pid: number; last_seen_at: number };
