...
import type { OrcCommand, OrcEvent } from "@orc/types";
...
import { type ConcurrencyGovernor, configureGovernor } from "./concurrency";
...
export class MastermindStateMachine {
  ...
  async run(opts: { userGoal: string }): Promise<{
    status: "done" | "failed";
    runBranch: string;
  }> {
    if (this.cfg.maxConcurrentWorkers)
    ...
    this.emit("CommandIssued", {
      ...
      to: "mastermind",
      commandType: "Run",
      commandPayload: { goal: opts.userGoal },
    });

    const runDir = path.join(
      getOrcPaths(this.cfg.repoRoot).runsDir,
      this.cfg.runId,
    );
    ...
    try {
      if (this.cfg.templatePath) {
        ...
        const prompt = await this.ctx.buildMastermindPrompt(
          opts.userGoal,
          this.cfg.runId,
        );
        ...
        sections = Array.isArray(parsed)
          ...
          : [{ id: "main", goal: opts.userGoal, numWorkers: 2, dependsOn: [] }];
      }
    } catch {
      sections = [
        { id: "main", goal: opts.userGoal, numWorkers: 2, dependsOn: [] },
    ...
    const runWtPath = path.join(
      getOrcPaths(this.cfg.repoRoot).worktreesDir,
      `run-${this.cfg.runId}`,
    );
    await addWorktreeForBranch(this.cfg.repoRoot, runWtPath, runBranch, {
      hydrateDependencies: false,
    }).catch((err) => {
      if (
        !err.message?.includes("already exists") &&
        !err.message?.includes("is already checked out")
      )
        throw err;
    });
    ...
    const sectionResults = new Map<
      string,
      { status: "done" | "failed"; winnerBranch: string }
    >();
    ...
    while (remaining.length > 0) {
      ...
      if (batch.length === 0) {
        const blockedByFailure = remaining.filter((s) =>
          s.dependsOn.some((d) => failed.has(d)),
        );
        ...
        this.emit("OrchestrationFailed", {
          reason: "circular or unresolvable section dependencies",
        });
      ...
      const batchResults = await Promise.allSettled(
        batch.map((section) => {
          this.emit("LeadDelegated", {
            leadId: `${section.id}-lead`,
    ...
    const failures = [...sectionResults.values()].filter(
      (r) => r.status === "failed",
    );
    if (failures.length > 0) {
      this.state = "failed";
      this.emit("OrchestrationFailed", {
        reason: `${failures.length} section(s) failed`,
      });
      return { status: "failed", runBranch };
    }
    ...
    const doMerge =
      this.cfg.doMerge ?? ((_target, source) => mergeBranch(runWtPath, source));
