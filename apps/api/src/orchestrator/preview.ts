...
async function codeFileForWorker(
  worktreePath: string,
  worktreeId: string,
): Promise<string> {
...
export async function generateStrudelPreviewUrl(
  worktreePath: string,
  worktreeId: string,
): Promise<string> {
  ...
  let code = "";
  try {
    code = await fs.readFile(path.join(worktreePath, codeFile), "utf-8");
  } catch {
    code = `// ${codeFile} not yet written`;
  }
...
export function stopPreview(db: Db, worktreeId: string): void {
  getSQLite(db)
    .prepare("UPDATE previews SET status='inactive' WHERE worktree_id=?")
    .run(worktreeId);
}
