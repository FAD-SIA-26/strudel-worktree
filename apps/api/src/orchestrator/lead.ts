...
export class LeadStateMachine {
  ...
  async run(): Promise<LeadResult> {
    ...
    this.emit("CommandIssued", {
      from: "mastermind",
      to: this.cfg.id,
      commandType: "SpawnLead",
      commandPayload: {},
    });

    const pmPrompt = this.ctx.buildPMPrompt(
      this.cfg.sectionId,
    ...
    try {
      ...
      workerPrompts = Array.isArray(parsed)
        ? parsed
        : Array.from(
            { length: this.cfg.numWorkers },
            () => this.cfg.sectionGoal,
          );
    } catch {
      workerPrompts = Array.from(
        { length: this.cfg.numWorkers },
        () => this.cfg.sectionGoal,
      );
    }

    const leadPlanPath = await createLeadPlan(
      this.cfg.repoRoot,
      this.cfg.runId,
      {
    ...
    const workers = workerPrompts.map(
      (prompt, i) =>
    ...
    const done = results
      ...
      .flatMap((x) => {
        if (x.r.status !== "fulfilled" || x.r.value.status !== "done") {
          return [];
        }
        return [
          {
    ...
    if (forceWinnerId && done.some((d) => d.workerId === forceWinnerId)) {
      ...
      const reviewPrompt = this.ctx.buildReviewerPrompt(
        this.cfg.sectionGoal,
        done,
      );
      try {
    ...
    this.emit("MergeRequested", {
      leadId: this.cfg.id,
      worktreeId: winner.workerId,
      targetBranch: this.cfg.runBranch,
    });
    this.state = "done";
    this.emit("LeadDone", {
