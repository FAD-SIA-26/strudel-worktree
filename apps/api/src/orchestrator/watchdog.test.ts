...
describe("Watchdog", () => {
  it("marks stalled worker when last_seen_at exceeds threshold", async () => {
    ...
    getSQLite(db)
      .prepare(
        `INSERT INTO tasks(id,type,parent_id,state,created_at,updated_at) VALUES('w1','worker','l1','running',${now},${now})`,
      )
      .run();
    getSQLite(db)
      .prepare(
        `INSERT INTO runs(id,entity_id,started_at,last_seen_at) VALUES('r1','w1',${now - 200_000},${now - 200_000})`,
      )
      .run();
