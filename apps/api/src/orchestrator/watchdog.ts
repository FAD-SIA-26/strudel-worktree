...
export class Watchdog extends EventEmitter {
  ...
  private poll(): void {
    ...
    for (const w of workers) {
      ...
      let pidAlive = false
      if (w.pid) { try { process.kill(w.pid, 0); pidAlive = true } catch {} }
      ...
      if (w.pid && pidAlive) {
        ...
        writeEvent(this.cfg.db, { entityId: w.id, entityType: 'worker', eventType: 'WorkerStalled',
          sequence: nextSeq(w.id), ts: now, payload: { lastSeenAt: lastSeen } } as OrcEvent, () => {})
        ...
        writeEvent(this.cfg.db, { entityId: w.id, entityType: 'worker', eventType: 'WorkerZombie',
          sequence: nextSeq(w.id), ts: now, payload: { pid: w.pid } } as OrcEvent, () => {})
