"use client";
import type { OrcEvent, OrchState } from "@orc/types";
...
function applyEvent(old: OrchState | undefined, ev: OrcEvent): OrchState {
    ...
    return {
      ...
      sections: old.sections.map((s) => ({
        ...
        workers: s.workers.map((w) =>
          w.id === ev.entityId
            ? { ...w, state: workerStateMap[ev.eventType] }
            : w,
        ),
      })),
    };
  ...
  if (leadStateMap[ev.eventType]) {
    const sectionId = ev.entityId.replace("-lead", "");
    return {
      ...old,
      sections: old.sections.map((s) =>
  ...
  if (ev.eventType === "LeadDelegated") {
    const sectionId = ev.payload.sectionId;
    ...
    return {
      ...
      sections: exists
        ? old.sections
        : [...old.sections, { id: sectionId, state: "idle", workers: [] }],
    };
  }
...
export function useOrchestration() {
  ...
  useEffect(() => {
    ...
    const unsub = orcWs.subscribe((ev) => {
      qc.setQueryData<OrchState>(["orchestration"], (old) =>
        applyEvent(old, ev),
      );
    });
    return () => {
