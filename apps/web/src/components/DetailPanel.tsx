...
function CodeBlock({
  children,
  color = "blue",
}: { children: string; color?: "blue" | "amber" }) {
  const palette =
    color === "amber"
  ...
  return (
    <code
      className={`block text-[11px] font-mono rounded px-3 py-2 border break-all leading-relaxed ${palette}`}
    >
      {children}
    </code>
  );
...
export function DetailPanel({
  ...
  const worker: WorkerInfo | undefined = sections
    .flatMap((s) => s.workers)
    .find((w) => w.id === selectedId);
  const workerCfg = worker
    ? (WORKER_STATE_CFG[worker.state] ?? WORKER_STATE_CFG.queued)
    : null;
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#1c2738] bg-[#0b0e14] flex-shrink-0 min-w-0">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span className="font-mono text-[12px] text-gray-200 truncate">
            {selectedId}
          </span>
          {workerCfg && (
            <div className="flex items-center gap-1 flex-shrink-0">
              <span className={`w-1.5 h-1.5 rounded-full ${workerCfg.dot}`} />
              <span className={`text-[10px] ${workerCfg.text}`}>
                {worker?.state}
              </span>
            </div>
          )}
        ...
        {tab === "compare" && isLead && section && (
          <div className="max-w-2xl space-y-2">
            <p className="text-[11px] text-gray-600 mb-4">
              Review each worker's output. Launch a preview to compare, then
              pick the winner.
            </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-mono text-[11px] text-gray-200 truncate">
                      {w.id}
                    </div>
