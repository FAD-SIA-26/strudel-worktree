// Savour - Tim Smith
// Genre: Electro/Art Rock
// BPM: 120
// Key: D Minor
// Source: github:eefano/strudel-songs-collection/savour.js
// Experimental electro with complex chord progressions

setcpm(120 / 2)

const song = "<0@24 0@24 1@21 2@24>"

const ch0rds = song.pick([
...
stack(
  chord(ch0rds).mode('above')
    .layer(x => x.anchor('c3').voicing().s("gm_bandoneon").hpf(200).lpf(4000).gain(.25).room(.4),
           x => x.anchor(song.pick(["f4", "a4", "e4"])).voicing().struct("x").clip(.8).s("gm_cello").att(.1).gain(.2).rel(.3).hpf(200).room(1)),

  n(song.pick(["[0@4 0 0]/2", "0", "[0@5 0]/2"])).chord(ch0rds).anchor("e2")
    .clip(.9).mode("root").voicing().transpose("<0@61 [0,12] [-1,11] [-3,9] [-5,7] [0,12] [-1,11] [-3,9] 0@25>")
    .s("gm_electric_bass_finger:2").lpf(300).gain(.8),

  n(song.pick(["[~ 0 1 2 1 0]/2", "2*2", "[~ ~ 0 _ 1 _]"]))
